...
int ajouter_un_contact_dans_rep(Repertoire *rep, Enregistrement enr)
{
	...
	{
		rep->tab[rep->nb_elts] = enr;    //si la condition est bien vérifiée, on ajoute alors le contact à la fin du tableau
		rep->nb_elts++;   //on augmente le nombre d'éléments
		return (OK);
	}
...
bool est_sup(Enregistrement enr1, Enregistrement enr2)
{
	...
	for (int i = 0; i < MAX_NOM; i++) {
		if (tolower(enr2.nom[i]) < tolower(enr1.nom[i])) return (true);   //on vérifie si la premiere lettre du deuxième contact est plus bas (=plus loin) dans l'alphabet que l'autre, si oui c'est bon
...
void trier(Repertoire *rep)
{
	...
	Enregistrement min;  // contact qui sert pour pour comparer
	for (int i = 0; i < rep->nb_elts - 1; i++) {  //on parcourt tout le tableau sauf le dernier contact car il sera trié automatiquement
...
int rechercher_nom(Repertoire *rep, char nom[], int ind)
{
	...
	while (trouve == false) {  //on va comparer a chaque nom du répertoire 
		...
		if (i == ind_fin) {
			return -1;  //si on est arrivé à la fin et qu'on n'a pas trouvé on return -1
		}
